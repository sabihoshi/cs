	...
	public partial class Return : Form
	{
		...
		private void UpdateLibrary()
		{
			userBorrowed = Interface.borrowers.Borrowers
			                        .Where(borrower => borrower.Name == Login.userName)
			                        .Select(borrower => borrower.Book).ToList();
			userLibrary = new Lib(Interface.library.Books.Where(book => userBorrowed.Contains(book.Name)).ToList());

			BookList.Items.Clear();
			List<Book> result = userLibrary.Books;
		...
		private void Return_Load(object sender, EventArgs e)
		{
			...
			foreach (string item in userLibrary.Books.SelectMany(b => b.Genre))
				genres.Add(item);
		...
		{
			Book selectedBook = userLibrary.Books.FirstOrDefault(b => b.Name == BookList.SelectedItem.ToString());
