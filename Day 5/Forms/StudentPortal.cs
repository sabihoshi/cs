...
namespace CIIT_Grading_System.Forms
{
    public partial class StudentPortal : Form
    {
        ...
        {
            Classroom Classroom = Login.User.userData.Classrooms.FirstOrDefault(c => c.Name.Equals(ClassroomList.Text));
        ...
        public string CalculateGrade(string classroomName, string studentName, string termName)
        {
            ...
            foreach (Graded item in student.Laboratory)
            {
                ...
                Grade.Table.Laboratory.Add(new List<string> { item.Name, $"{item.Score}/{item.Total}", "+2" });
            }
            ...
            Grade.Lectures.Percentage =
                (Grade.Lectures.Attendance.Percentage * 0.15 + Grade.Lectures.Exams.Percentage * 0.75) * 0.25;
            ...
            for (var i = 0; i < lower.Count; i++) tableList[i].AddRange(lower[i]);

            for (var i = 0; i < Grade.Table.HandsOn.Count; i++) tableList[i].AddRange(Grade.Table.HandsOn[i]);
            ...
            tableOutput.Add(
                $"|**Total**|{Grade.Lectures.Percentage:0.00}%| | |{Grade.Laboratory.Percentage:0.00}%| | |{Grade.HandsOn.Percentage:0.00}%| |");
        ...
        {
            Login.User.userData.Classrooms.Add(new Classroom(Interaction.InputBox("Enter a new classroom name",
                "Create new classroom", "Class #")));
        ...
        private void changePasswordToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Login.userLogin.User.SingleOrDefault(u => u.Username == Login.userName).Password =
                Interaction.InputBox("Enter a new password", "Password Change", userPass);
