...
namespace Quiz.Forms
{
	public partial class Answer : Form
	{
		...
		private void openAQuizToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(@"Invalid file", @"Open file", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
		...
		private void Next_Click(object sender, EventArgs e)
		{
			...
			if (pageCurrent == pageMax)
			{
				if (answers.Any(c => c == -1))
				{
					...
					if (MessageBox.Show($"Your answer in {result} is empty", "Continue?", MessageBoxButtons.OKCancel) ==
					    DialogResult.Cancel)
						return;
				}
