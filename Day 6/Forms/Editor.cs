...
namespace Quiz.Forms
{
	public partial class Editor : Form
	{
		...
		private void UpdateObject()
		{
			...
			currentQuestion.Choices = new[] {ChoiceA.Text, ChoiceB.Text, ChoiceC.Text, ChoiceD.Text};
		...
		private void saveToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (Title.Text == "")
				...
				MessageBox.Show(@"Please have at least one question!", @"Save file", MessageBoxButtons.OK,
				...
				try
				{
					...
					if (SaveFile.ShowDialog() == DialogResult.OK)
					{
						File.WriteAllText(SaveFile.FileName,
							JsonConvert.SerializeObject(Questionnaire, Formatting.Indented));
						MessageBox.Show(@"File saved.", @"Save file", MessageBoxButtons.OK, MessageBoxIcon.Information);
					}
				}
				catch (JsonWriterException)
				{
					MessageBox.Show(@"Invalid file", @"Save file", MessageBoxButtons.OK, MessageBoxIcon.Error);
				}
		}
